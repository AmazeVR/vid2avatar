...
def main(args):
    ...
    if args.mode == 'mask':
        ...
        cam = PerspectiveCamera(focal_length_x=torch.tensor(cam_intrinsics[0, 0], dtype=torch.float32),
                                focal_length_y=torch.tensor(
                                    cam_intrinsics[1, 1], dtype=torch.float32),
        ...
        refined_smpl_mask_paths = sorted(
            glob.glob(f"{refined_smpl_mask_dir}/*.png"))
        ...
        T_hip = smpl_model.get_T_hip(betas=torch.tensor(
            smpl_shape)[None].float().to(device)).squeeze().cpu().numpy()
    ...
    for idx, img_path in enumerate(tqdm(img_paths)):
        ...
        if args.mode == 'mask' or args.mode == 'refine':
            seq_file = np.load(romp_file_paths[idx], allow_pickle=True)[
                'results'][()]
            ...
            if len(seq_file['smpl_thetas']) >= 2:
                ...
                for i in range(len(seq_file['smpl_thetas'])):
                    dist.append(np.linalg.norm(seq_file['joints'][i].mean(
                        0) - last_j3d.mean(0, keepdims=True)))
            ...
            if args.mode == 'refine':
                ...
                openpose_j2d = torch.tensor(
                    openpose[:, :2][None], dtype=torch.float32, requires_grad=False, device=device)
                openpose_conf = torch.tensor(
                    openpose[:, -1][None], dtype=torch.float32, requires_grad=False, device=device)
                ...
                optimizer = torch.optim.Adam(
                    opt_params, lr=2e-3, betas=(0.9, 0.999))
                if idx == 0:
                ...
                for it in loop:
                    ...
                    smpl_joints_2d = cam(torch.index_select(
                        smpl_output.joints, 1, smpl2op_mapping))

                    loss = dict()
                    loss['J2D_Loss'] = joints_2d_loss(
                        openpose_j2d, smpl_joints_2d, openpose_conf)
                    loss['Temporal_Loss'] = pose_temporal_loss(
                        last_pose[0], opt_pose)
                    w_loss = dict()
                    for k in loss:
            ...
            smpl_mesh = trimesh.Trimesh(
                smpl_verts, smpl_model.faces, process=False)
            R = torch.tensor(cam_extrinsics[:3, :3])[None].float()
            T = torch.tensor(cam_extrinsics[:3, 3])[None].float()
            rendered_image = render_trimesh(renderer, smpl_mesh, R, T, 'n')
            if input_img.shape[0] < input_img.shape[1]:
                rendered_image = rendered_image[abs(
                    input_img.shape[0]-input_img.shape[1])//2:(input_img.shape[0]+input_img.shape[1])//2, ...]
            else:
                rendered_image = rendered_image[:, abs(
                    input_img.shape[0]-input_img.shape[1])//2:(input_img.shape[0]+input_img.shape[1])//2]
            ...
            if args.mode == 'mask':
                cv2.imwrite(os.path.join(
                    f'{DIR}/{seq}/init_mask', '%04d.png' % idx), valid_mask*255)
            elif args.mode == 'refine':
                output_img = (
                    rendered_image[:, :, :-1] * valid_mask + input_img * (1 - valid_mask)).astype(np.uint8)
                cv2.imwrite(os.path.join(
                    f'{DIR}/{seq}/init_refined_smpl', '%04d.png' % idx), output_img)
                cv2.imwrite(os.path.join(
                    f'{DIR}/{seq}/init_refined_mask', '%04d.png' % idx), valid_mask*255)
                last_pose.pop(0)
                ...
                pkl.dump(smpl_dict, open(os.path.join(
                    f'{DIR}/{seq}/init_refined_smpl_files', '%04d.pkl' % idx), 'wb'))
        elif args.mode == 'final':
            input_img = cv2.resize(
                input_img, (input_img.shape[1] // scale_factor, input_img.shape[0] // scale_factor))
            ...
            cv2.imwrite(os.path.join(
                save_dir, 'image/%04d.png' % idx), input_img)
            ...
            smpl_output = smpl_model(betas=torch.tensor(smpl_shape)[None].float().to(device),
                                     body_pose=torch.tensor(smpl_pose[3:])[
                None].float().to(device),
                global_orient=torch.tensor(smpl_pose[:3])[
                None].float().to(device),
            ...
            target_extrinsic[:3, -1] = target_extrinsic[:3, -
                                                        1] - (target_extrinsic[:3, :3] @ normalize_shift)
            # T = T - R x
    ...
    if args.mode == 'final':
        ...
        np.save(os.path.join(save_dir, 'normalize_trans.npy'),
                np.array(output_trans))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, default='custom',
                        help="custom video or dataset video")
    # sequence name
    parser.add_argument('--seq', type=str)
    # gender
    parser.add_argument('--gender', type=str,
                        help="gender of the actor: MALE or FEMALE")
    # mode
    parser.add_argument(
